...
window.ZigMessagesNotify = (function () {
    const STORAGE_KEY = 'zig_last_incoming_message_id';
    const POLL_MS = 12000;
    ...
    const getConfig = () => {
        ...
        return {
            ...
            pollUrl: body?.dataset.messagesPollUrl || '/api/messages/unread',
            appUrl: (body?.dataset.appUrl || '').replace(/\/$/, ''),
            notifyTitle: body?.dataset.messagesNotifyTitle || 'Nouveau message',
            notifyEnable: body?.dataset.messagesNotifyEnable || 'Activer les notifications',
            notifyDismiss: body?.dataset.messagesNotifyDismiss || 'Plus tard',
        };
    };
    ...
    const updateBadges = (count) => {
        ...
        document.querySelectorAll('[data-messages-nav]').forEach((link) => {
            link.classList.toggle('has-unread', count > 0);
        });
    };

    const ensureToastRoot = () => {
        let root = document.getElementById('messagesToastRoot');
        if (!root) {
            root = document.createElement('div');
            root.id = 'messagesToastRoot';
    ...
    const showInAppToast = (payload) => {
        ...
        toast.href = payload.url || '/messages';
        toast.className = 'messages-toast';
        toast.innerHTML = ''
            + '<span class="messages-toast-icon" aria-hidden="true"><i class="bi bi-chat-dots-fill"></i></span>'
    ...
    const showBrowserNotification = (payload) => {
        ...
        const title = payload.sender_name
            ? payload.sender_name + ' — ' + cfg.notifyTitle
            : cfg.notifyTitle;
        const notification = new Notification(title, {
            body: payload.body || '',
            icon: cfg.appUrl ? cfg.appUrl + '/assets/img/icon-192.png' : undefined,
            tag: 'zig-message-' + (payload.conversation_id || payload.id || Date.now()),
            renotify: true,
        });
    ...
    const handleIncoming = (payload, options = {}) => {
        ...
        if (messageId <= lastIncomingId && !options.force) {
            return;
        }

        const threadRoot = document.getElementById('messagesThread');
        const activeConversationId = threadRoot
            ? parseInt(threadRoot.dataset.conversationId || '0', 10)
            : 0;
        const sameOpenThread = activeConversationId > 0
        ...
        if (!sameOpenThread) {
            ...
            document.dispatchEvent(new CustomEvent('zig:new-message', { detail: payload }));
        }

        lastIncomingId = Math.max(lastIncomingId, messageId);
    ...
    const poll = async () => {
        ...
        try {
            ...
            document.dispatchEvent(new CustomEvent('zig:messages-unread', {
                detail: {
                    count: json.count || 0,
    ...
    const setupPermissionBanner = () => {
        ...
        const page = document.querySelector('[data-messages-enable-banner]');
...
document.addEventListener('DOMContentLoaded', () => {
    window.ZigMessagesNotify.bootstrap();
});
